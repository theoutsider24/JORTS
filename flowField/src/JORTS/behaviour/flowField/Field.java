...
import static JORTS.common.Constants.SHOW_FLOW;
...
public class Field  implements Drawable,Observer{
	...
	{				
		...
		*///arg0.draw(sprite);	
		...
		for(int i=0;i<GRID_SIZE;i++)
			for(int j=0;j<GRID_SIZE;j++)
			{
				if(cells[i][j].getPosition().x>topLeft.x&&cells[i][j].getPosition().y>topLeft.y&&
						cells[i][j].getPosition().x<bottomRight.x&&cells[i][j].getPosition().y<bottomRight.y)
				{
		...
		for(int i=0;i<GRID_SIZE;i++)
			for(int j=0;j<GRID_SIZE;j++)
			{
				RectangleShape r=cells[i][j].getVectorVisualistation();
					if(CommonFunctions.isOnScreen(arg0,cells[i][j].hitBox))
					{
						x++;
						arg0.draw(r);
						arg0.draw(cells[i][j].integrationText);
	...
	public boolean hasLineOfSightWithAssignment(FlowCell goal,FlowCell c)
	{
		...
		{
			//for(int i=0;i<pts.size()&&los;i++)
			for(int i=pts.size()-1;i>=0&&los;i--)
			{
				int p[] = pts.get(i);
				...
				try{
				int p2[] = pts.get(i+1);
				cells[p[0]][p[1]].integration=cells[p2[0]][p2[1]].integration+(int)(CommonFunctions.getDist(cells[p[0]][p[1]].getCenter(),cells[p2[0]][p2[1]].getCenter())/CELL_SIZE)+cells[p[0]][p[1]].cost;
	...
	{		
		...
		for(int i=0;i<GRID_SIZE;i++)
		{
			for(int j=0;j<GRID_SIZE;j++)
			{
				if(cells[i][j].los || !cells[i][j].isValid || !cells[i][j].isOpen())
					continue;
